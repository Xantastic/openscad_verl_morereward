...
def call_sandbox_api(
    ...
    last_error = None
    response = None  # Initialize response variable for proper cleanup
    ...
    try:
        for attempt in range(MAX_RETRIES):
            try:
                ...
                # Ensure proper resource cleanup of any previous response
                ...
                if response.status_code == 504:
                    ...
                    if attempt < MAX_RETRIES - 1:
                        # Use linear backoff for simplicity
                ...
                last_error = f"{log_prefix}API Request Timeout: {e}"
                logger.warning(last_error)
                if attempt < MAX_RETRIES - 1:
                    delay = INITIAL_RETRY_DELAY * (attempt + 1)
                    logger.info(f"{log_prefix}Retrying after {delay} seconds...")
                ...
                last_error = f"{log_prefix}API Connection Error: {e}"
                logger.warning(last_error)
                if attempt < MAX_RETRIES - 1:
                    delay = INITIAL_RETRY_DELAY * (attempt + 1)
                    logger.info(f"{log_prefix}Retrying after {delay} seconds...")
...
def check_correctness(
    ...
    processing test cases concurrently with improved resource management.
    ...
    try:
        ...
        max_workers = min(32, os.cpu_count() * 2)  # Reduced from *5 to *2 for better resource management
        executor = concurrent.futures.ThreadPoolExecutor(max_workers=max_workers)
        
        # Submit all tasks with proper error handling
        future_to_index = {}
        for i, stdin_data in enumerate(inputs):
            try:
                ...
                logger.error(f"Failed to submit test case {i}: {e}")
                results[i] = -1
                metadata_list[i] = {
                    "case_index": i,
                    "input": str(stdin_data),
                    "expected_output": str(expected_outputs[i]) if expected_outputs[i] else None,
                    "api_request_error": f"Task submission failed: {e}",
                    "status": "submission_error",
                }

        # Process results as they complete with proper cleanup
